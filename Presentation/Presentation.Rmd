...
\end{equation*}
- If there were no outliers, result is $Z_t$: $\pi(B) X_t = Z_t$
...
- Once outlier is detected, effect can be subtracted to obtain adjusted series
...
tso(y, cval = NULL, delta = 0.7,
    ...
    args.tsmethod = NULL)
```
- ~~Sometimes~~ Often fails or takes a long time
- Might need to increase `maxit.iloop`
- Might help to specify model to fit using `arima` instead of
letting `auto.arima` choose the model
