...
\begin{equation}
\hat{\tau} = \frac{\hat{\omega}}{\hat{\sigma}_a / \sqrt{\sum_{t = t_1}^{n} x^2_t}}
\end{equation}

where $\hat{\sigma}_a$ is an estimate of $\sqrt{\text{Var}(Z_t)}$ (see
...
is not often the case, have found): the terrorist attacks on 9/11 and the
...
#### Example 1: Outliers in weekly closings of the Dow-Jones industrial average, July 1971- August 1974.
```{r,fig.align='left',echo=TRUE,eval=TRUE}
library('fma')
dj <- read.csv('../data/weekly-closings-of-the-dowjones-.csv')
plot(as.numeric(dj[,2]),main="Weekly closings of the Dow-Jones industrial average",type='l',
	 xlab=" Weeks ", ylab="Closings")

 
#data(Nile)
dj.ts <-ts(dj$Weekly.closings.of.the.Dow.Jones.industrial.average..July.1971...August.1974,frequency = 1)
fit<- auto.arima(dj.ts)
```
Applying the two stage outlier detection process using `tsoutliers` function
generates

```{r,eval=TRUE,echo=TRUE}
dj.outliers <- tso(dj.ts,types=c("AO","LS","TC","IO"),maxit = 30)
plot(dj.outliers)
```
The outlier detected are 
```{r, echo=TRUE,results='markup'}
# Outlier values 
print(dj.outliers$outliers)
```
The main detected outliers are `level shift (LS)` at index 73 and 84, `Additive outlier (AO)` at 96, 163, 91 and `Temporal Change (TC)` at 111.    Additinaly, the fitted ARIMA model is ARIMA( 0,1,0).

```{r, echo=TRUE,results='markup'}
# Outlier values 
print(dj.outliers$fit)
```


#### Example 2:  Outlier in measurement of the annual flow of the river Nile at Ashwan from 1871-1970.
...
#### Example 3. Outliers in price of chicken in USA from 1924 to 1993. 
